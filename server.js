...
// ── Upload image + run Tesseract server-side → return OCR text to client ──────
app.post('/api/upload-ocr', (req, res) => {
  const { base64, filename } = req.body;
  if (!base64 || !filename) return res.status(400).json({ error: 'Missing base64 or filename' });
  try {
    ...
    // Run Tesseract (Thai + English) on the uploaded image
    const ocrArgs = ['bash', '-c',
      `tesseract "${wslPath}" stdout --psm 6 -l tha+eng 2>/dev/null; rm -f "${wslPath}"`
    ];
    let ocrText = '';
    try {
    ...
    console.log(`[upload-ocr] ${safeFilename} → ${trimmed.length} chars OCR`);
    res.json({ ok: true, ocrText: trimmed });
  } catch (e) {
    console.error('[upload-ocr] ✕', e.message);
    res.status(500).json({ error: e.message });
  }
});
