...
export const STAGES = [
  ...
  { id:2, icon:"📚", color:"#f87171", glow:"rgba(248,113,113,0.12)",
    ...
    sub_en:"Credential Recovery + Forensics", sub_th:"กู้ข้อมูลรับรองและนิติวิทยาศาสตร์",
    objective_en:"Obtain config_backup.enc, decode it with base64, extract the API token and flag hidden inside.",
    objective_th:"ดาวน์โหลด config_backup.enc ถอดรหัสด้วย base64 แล้วดึง API token และ flag",
    scenario_en:"A config backup was leaked from the rogue agent's workspace. The encoding scheme is intentionally obscured.",
    scenario_th:"ไฟล์ backup configuration รั่วไหลจาก workspace ของตัวแทนอันธพาล รูปแบบการเข้ารหัสถูกซ่อนไว้",
    hint_en:"Binary-to-text encodings can be layered. Base64 → Hex → JSON. Each layer yields more data.",
    hint_th:"การเข้ารหัสสามารถซ้อนกันได้หลายชั้น Base64 → Hex → JSON แต่ละชั้นให้ข้อมูลเพิ่ม",
...
const checkWin = (id, aiRes, cookies) => {
  ...
  // Stage 2
  if (id === 2) {
    if (ai.includes("flag: credential_breach_9x2")) return true;
    if (ai.includes("flag: vault_9k2m_breach")) return true;
    if ((ai.includes("clearance") || ai.includes("granted")) &&
        (ai.includes("credential") || ai.includes("vault") || ai.includes("api_token"))) return true;
  }
  ...
  return (
    <div style={S.root}>
      ...
      <button
        ...
        style={{ position:"fixed", top:12, right:16, zIndex:9999,
            ...
            {STAGES.map((st, idx) => {
              ...
              return (
                <button key={st.id} onClick={() => ok && setActive(st.id)} disabled={!ok}
                  className="nav-btn"
                  style={{ ...S.navBtn, animationDelay:`${idx * 0.07}s`,
